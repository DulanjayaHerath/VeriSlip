...
def get_synthetic_sample(
    bank_code: str = Query("COMBANK", pattern="^(COMBANK|SAMPATH|BOC|HNB|SEYLAN|GENERIC_CEFTS)$"),
    tampered: bool = Query(False, description="Whether to generate a doctored/tampered slip"),
    tamper_type: str = Query("ALTER_AMOUNT", pattern="^(ALTER_AMOUNT|ALTER_REFERENCE|SPLICE_REFERENCE)$")
):
    """
    ...
    """
    effective_tamper_type = "ALTER_REFERENCE" if tamper_type == "SPLICE_REFERENCE" else tamper_type
    ...
    if tampered:
        ...
        final_img, final_meta = generator.generate_tampered_slip(
            ...
            metadata=metadata,
            tamper_type=effective_tamper_type,
