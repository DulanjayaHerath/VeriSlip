...
notebook = {
 "cells": [
  ...
  },
  {
   ...
   "outputs": [],
   "source": [
    "# Search for dataset folder or zip in Kaggle working / input directory\n",
    "DATASET_ROOT = 'verislip_dataset'\n",
    "\n",
    "if not os.path.exists(DATASET_ROOT):\n",
    "    # Check if a zip archive exists in input or root\n",
    "    possible_zips = glob.glob('/kaggle/input/**/verislip*.zip', recursive=True) + glob.glob('*.zip')\n",
    "    if possible_zips:\n",
    ...
    "if csv_candidates:\n",
    "    df_meta = pd.read_csv(csv_candidates[0])\n",
    "    DATASET_ROOT = os.path.dirname(csv_candidates[0])\n",
    "    print(f'✓ Found metadata at {csv_candidates[0]} with {len(df_meta)} samples.')\n",
    "    display(df_meta.head(4))\n",
    "else:\n",
    "    print('⚠️ Please upload verislip_dataset.zip into the notebook workspace.')"
   ]
  },
  {
   "cell_type": "markdown",
...
with open('/Users/chirana/IdeaProjects/VeriSlip/notebooks/VeriSlip_DualStream_Training.ipynb', 'w') as f:
    json.dump(notebook, f, indent=1)

print("Created notebooks/VeriSlip_DualStream_Training.ipynb successfully!")
